...
export default function OrdersScreen() {
  const { user } = useAuth();
  const { orders, getGigById } = useApp();
  const router = useRouter();

  const [activeTab, setActiveTab] = useState<'all' | 'pending' | 'confirmed' | 'completed'>('all');
  const onChangeTab = (tabId: string) => setActiveTab(tabId as 'all' | 'pending' | 'confirmed' | 'completed');

  // Filter for this client from live Firestore data
  ...
  const filteredOrders = useMemo(
    () =>
      activeTab === 'all'
        ? clientOrders
        : clientOrders.filter(order => order.status === activeTab),
    [clientOrders, activeTab],
  );

  const handleOrderPress = (orderId: string) => {
    router.push(`/(client)/order/${orderId}` as any);
  };
