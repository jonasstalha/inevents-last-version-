...
import { ActivityIndicator, Alert, Animated, Linking, Modal, Platform, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function PhoneVerificationModal({
  ...
  const handleSendCode = useCallback(async () => {
    ...
    try {
      const { formattedPhone: normalizedPhone, verificationCode } = await initiatePhoneVerification(phoneNumber);
      setFormattedPhone(normalizedPhone);
      setCodeSent(true);
      setCountdown(60);

      if (verificationCode && Platform.OS !== 'web') {
        const message = `Your inEvent verification code is ${verificationCode}`;
        ...
        const webUrl = `https://api.whatsapp.com/send?phone=${whatsappPhone}&text=${encodeURIComponent(message)}`;
                  ...
                  >
                    {isSendingCode ? (
                      ...
                      <Text style={styles.buttonText}>
                        {Platform.OS !== 'web' ? 'Send via WhatsApp' : 'Send Verification Code'}
                      </Text>
                    )}
                  </TouchableOpacity>
                ) : (
