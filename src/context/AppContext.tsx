...
const MOCK_GIGS: Gig[] = [
  {
    ...
    description:
      "Professional live music for your special event. Solo performer with vocals and acoustic guitar.",
    basePrice: 300,
    images: [
      "https://images.pexels.com/photos/96380/pexels-photo-96380.jpeg?auto=compress&cs=tinysrgb&w=600",
    ],
    category: "Music",
    options: [
      {
        id: "opt1",
        title: "Extra Hour",
        description: "Add an additional hour to the performance",
        price: 100,
      },
      {
        id: "opt2",
        title: "Song Requests",
        description: "Prepare up to 5 song requests in advance",
        price: 50,
      },
    ],
    rating: 4.8,
  ...
  },
  {
    ...
    description:
      "Capture your special moments with a professional photo session.",
    basePrice: 250,
    images: [
      "https://images.pexels.com/photos/3062541/pexels-photo-3062541.jpeg?auto=compress&cs=tinysrgb&w=600",
    ],
    ...
    options: [
      {
        id: "opt1",
        title: "Extra Hour",
        description: "Add an additional hour to the session",
        price: 100,
      },
      {
        id: "opt2",
      ...
      },
      {
        ...
        description: "Get select photos the same day",
        price: 50,
      },
    ],
    rating: 4.7,
  ...
  },
  {
    ...
    description:
      "Exquisite gourmet catering for your event with customizable menu options.",
    basePrice: 500,
    images: [
    ...
    options: [
      {
        id: "opt1",
        title: "Premium Menu",
        description: "Upgrade to our premium selection",
        price: 200,
      },
      {
        id: "opt2",
        title: "Bar Service",
        description: "Add professional bar service with mixologists",
        price: 300,
      },
      {
        id: "opt3",
        title: "Dessert Station",
        description: "Add a gourmet dessert station",
        price: 150,
      },
    ],
    rating: 4.9,
...
const MOCK_ORDERS: Order[] = [
  {
    ...
    specialRequests:
      'Would like to include "Somewhere Over the Rainbow" as one of the song requests.',
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("Error loading saved artists:", e);
      }
    })();
  }, []);

  // ── Clear saved artists on logout ────────────────────────────────────────────
  useEffect(() => {
    if (!user) {
      setSavedArtists([]);
      console.log("🧹 Cleared saved artists on logout");
    }
  }, [user]);
  ...
  const saveArtist = async (artistId: string) => {
    ...
    try {
      await AsyncStorage.setItem("@saved_artists", JSON.stringify(next));
    } catch {}
  };
  ...
  const getSavedArtists = () =>
    savedArtists
      .map((id) => MOCK_ARTISTS.find((a) => a.id === id))
  ...
  const getGigById = (id: string) => MOCK_GIGS.find((g) => g.id === id);
