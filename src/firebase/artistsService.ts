...
export const addServiceToFirebase = async (artistId: string, service: ServiceData, serviceId?: string) => {
  try {
    ...
    if (!service.locationName) throw new Error('Service location is required');
    ...
    if (!artistId || artistId === 'undefined' || artistId === 'null') {
      throw new Error('Invalid artist ID. Please log in again.');
    }
    ...
    const newService = {
      ...
      locationName: String(service.locationName).trim(),
    ...
    if (service.images && service.images.length > 0) {
      console.log('[addServiceToFirebase] Uploading images to Firebase Storage...');
      ...
      try {
        ...
        // Don't throw - service was created, just without images
      }
    } else {
      console.log('[addServiceToFirebase] No images to upload');
    }
    ...
    if (service.video) {
      ...
      try {
        const videoAsset = {
          ...
          height: 0,
          type: 'video' as const,
          fileName: 'service_video.mp4',
        };
        
        videoUrl = await uploadServiceVideo(videoAsset, artistId, docRef.id);
        console.log('[addServiceToFirebase] Video uploaded successfully, URL:', videoUrl);
      } catch (uploadError) {
        console.error('[addServiceToFirebase] Error uploading video:', uploadError);
    ...
    // Return the created service with its ID and all media URLs
    return { 
      id: docRef.id, 
      ...newService, 
      ...updateData 
...
export const fetchArtistsFromFirebase = async (): Promise<Artist[]> => {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      // Use store name if available, otherwise use real name
      const artistName = userData.storeName || userData.name || userData.fullName || userData.displayName || 'Professional Artist';
