...
const isRemoteMediaUrl = (value: string): boolean =>
  value.startsWith('http://') || value.startsWith('https://') || value.startsWith('data:');

const isLocalMediaUri = (value: string): boolean =>
  value.startsWith('file://') || value.startsWith('content://');

const resolveLegacyMediaValue = async (rawValue: unknown): Promise<string | null> => {
  if (typeof rawValue !== 'string') return null;

  const value = rawValue.trim();
  if (!value) return null;
  if (isRemoteMediaUrl(value)) return value;
  if (isLocalMediaUri(value)) return null;

  try {
    return await getDownloadURL(storageRef(storage, value));
  } catch (error: any) {
    console.error('[ArtistServices] Failed to resolve legacy media path:', value, error?.code, error?.message);
    return null;
  }
};

/**
 * Migrates legacy media fields for an artist's services.
 * Converts storage paths to download URLs and drops unusable local URIs.
 */
export const migrateLegacyServiceMediaUrlsForArtist = async (artistId: string) => {
  ...
  for (const serviceDoc of snapshot.docs) {
    ...
    const resolvedCandidates = await Promise.all(rawCandidates.map(resolveLegacyMediaValue));
    const resolvedImages = Array.from(new Set(resolvedCandidates.filter((v): v is string => typeof v === 'string' && !!v)));
    ...
    const nextCover = resolvedImages[0] || null;
    const needsUpdate =
      JSON.stringify(currentImages) !== JSON.stringify(resolvedImages) ||
      data?.image !== nextCover ||
      data?.cover !== nextCover;

    if (!needsUpdate) continue;

    await updateDoc(doc(db, 'users', artistId, 'services', serviceDoc.id), {
      images: resolvedImages,
      image: nextCover,
      cover: nextCover,
      imageUrl: nextCover,
      coverUrl: nextCover,
      mediaMigratedAt: new Date(),
    });
