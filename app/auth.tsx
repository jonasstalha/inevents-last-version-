    ...
    <View style={styles.inputContainer}>
      <View style={styles.inputWrapper}>
        <TextInput
          ...
          secureTextEntry={
            shouldShowPasswordToggle ? isPasswordHidden : secureTextEntry
          }
...
export default function AuthScreen() {
  ...
  const navigateAfterAuth = async (fallbackRoute: string) => {
    try {
      const storedRedirect = await AsyncStorage.getItem(
  ...
  const isArtistFormValid = () => {
    ...
    return (
      storeName.trim() !== "" && city.trim() !== "" && categories.length > 0
    );
  };
  ...
  const handleGoogleSignIn = async (authentication: any) => {
    try {
      ...
      try {
        ...
        console.error("Google credential authentication error:", firebaseError);
        const authErrorMessage = getFriendlyAuthErrorMessage(firebaseError);
  ...
  const handleGoogleRoleSelection = async () => {
    try {
      ...
      setLoading(false);
      console.log(
        `✅ Google account created in Firestore! User role: ${userRole}`,
      );
  ...
  const handleGoogleArtistRegistration = async () => {
    try {
      ...
      if (!storeName.trim()) {
        Alert.alert(
          "Validation Error",
          "Please enter your store/business name",
        );
        return;
      }
      if (!city.trim()) {
    ...
    try {
      ...
      if (isLogin) {
        ...
        if (userRole === "artist" && !isArtistFormValid()) {
          Alert.alert(
            "Validation Error",
            "Please fill in all artist information including store name, city, and at least one category",
          );
          return;
        }
  ...
  const toggleAuthMode = () => {
    setIsLogin(!isLogin);
    // Clear form errors and temporary phone state when switching modes
    setEmailError("");
    setPasswordError("");
    setConfirmPasswordError("");
    setPhoneError("");
