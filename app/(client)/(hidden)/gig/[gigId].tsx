...
export default function ServiceDetailScreen() {
  ...
  const saveDraftAndRedirectToAuth = async (overrides?: {
    ...
    try {
      ...
      console.warn(
        "Failed to save gig draft before auth redirect:",
        storageError,
      );
    }
  ...
  useEffect(() => {
    const restoreDraft = async () => {
      ...
      try {
        ...
        if (parsedDraft?.serviceQuantities)
          setServiceQuantities(parsedDraft.serviceQuantities);
        if (typeof parsedDraft?.customMessage === "string")
          setCustomMessage(parsedDraft.customMessage);
        ...
        if (typeof parsedDraft?.couponApplied === "boolean")
          setCouponApplied(parsedDraft.couponApplied);
        if (typeof parsedDraft?.discount === "number")
          setDiscount(parsedDraft.discount);
        if (typeof parsedDraft?.showOfferForm === "boolean")
          setShowOfferForm(parsedDraft.showOfferForm);
      ...
      try {
        ...
        const resolveStorageUrl = async (uri?: string | null) => {
          ...
          if (value.startsWith("file://") || value.startsWith("content://")) {
            console.error(
              "Invalid persisted local media URI in service document:",
              value,
            );
            return null;
          }
        ...
        console.log("RAW image fields from Firestore:", {
          cover: serviceAny?.cover,
          image: serviceAny?.image,
          images: serviceAny?.images,
          imageUrl: serviceAny?.imageUrl,
          coverUrl: serviceAny?.coverUrl,
        });
        const rawServiceImages = Array.from(
          new Set(
            [
              serviceAny.cover,
              serviceAny.image,
        ...
        const rawServiceVideos = Array.from(
          new Set(
            [
              ...
              ...(Array.isArray(serviceAny.videoUrls)
                ? serviceAny.videoUrls
                : []),
              ...(Array.isArray(serviceAny.videoUris)
                ? serviceAny.videoUris
                : []),
            ].filter(Boolean),
          ),
        );
        ...
        if (service.userId) {
          try {
            ...
            if (artist) {
              const resolvedProfileImage = await resolveStorageUrl(
                artist.profileImage,
              );
              setProviderData({
                ...artist,
            ...
            return {
              ...
              date: commentAny.createdAt
                ? typeof commentAny.createdAt === "string"
                  ...
                  : commentAny.createdAt.toDate?.()?.toLocaleDateString() ||
                    "Recently"
                : "Recently",
            };
          });
  ...
  const actualImageSources = (
    Array.isArray(serviceData?.images) && serviceData.images.length > 0
      ? serviceData.images
      : [serviceData?.cover, serviceData?.image].filter(Boolean)
  ) as string[];
  ...
  const defaultServiceData = {
    ...
    provider: {
      name:
        providerData?.storeName ||
        providerData?.name ||
        serviceData?.artistName ||
        "Service Provider",
      avatar:
        providerData?.profileImage || providerData?.avatar || DEFAULT_AVATAR,
  ...
  const handleServiceChange = (change: number) => {
    setServiceQuantities((prev) => {
      const newQty = Math.max(
        ...
        Math.min(services.service.max, prev.service + change),
      );
  ...
  const getExtraPrice = (extra: any) => {
    if (!extra) return 0;
    return typeof extra.price === "number"
      ? extra.price
      : parseFloat(String(extra.price || 0));
  };
  ...
  const extrasList = useMemo(
    () => getExtrasCandidates(serviceData),
    [serviceData],
  );
  ...
  const calculateItemsTotal = () => {
    ...
    return serviceData.items.reduce(
      (total: number, item: any, index: number) => {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      if (!serviceData) {
        Alert.alert(
          "Error",
          "Service data is still loading. Please wait and try again.",
        );
        return;
      }
      ...
      if (!artistId) {
        ...
        Alert.alert(
          "Error",
          "Cannot create order: Artist ID is missing. Please try again.",
        );
        return;
      }
      ...
      await AsyncStorage.removeItem(getDraftStorageKey()).catch(
        () => undefined,
      );
    } catch (error: any) {
  ...
  const handleSendOffer = async () => {
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ Could not fetch user profile, using defaults:",
          profileError,
        );
      }
      ...
      if (!serviceData) {
        ...
        Alert.alert(
          "Error",
          "Service data is not loaded. Please wait for the page to load completely.",
        );
        return;
      }
      ...
      if (error.code === "permission-denied") {
        errorMsg =
          "Permission denied. You may not have permission to create orders.";
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (discountValue > 0) {
        ...
        Alert.alert(
          "✅ Promo Applied!",
          `You saved ${discountValue} MAD on your order!`,
        );
      } else {
        setDiscount(0);
        setCouponApplied(false);
      ...
      >
        <Animated.ScrollView
          ...
          contentContainerStyle={{
            paddingBottom: TOTAL_TAB_BAR_HEIGHT + 120 + insets.bottom,
          }}
            ...
            <View style={styles.mainContent}>
                ...
                <View style={styles.titleRatingRow}>
                  <View style={styles.starInline}>
                    ...
                    <Text style={styles.ratingNumber}>
                      {defaultServiceData.rating}
                    </Text>
                  </View>
                  <Text style={styles.ratingDot}>·</Text>
                  <Text style={styles.ratingMeta}>
                    {defaultServiceData.reviewCount} reviews
                  </Text>
              ...
              <View style={styles.providerSection}>
                <View style={styles.providerCard}>
                  <Image
                    source={{ uri: defaultServiceData.provider.avatar }}
                    ...
                    <View style={styles.providerNameRow}>
                      ...
                      <View style={styles.levelBadge}>
                        ...
                        <Text style={styles.levelText}>
                          {defaultServiceData.provider.level}
                        </Text>
                      </View>
                    </View>
              ...
              <View style={styles.descriptionSection}>
                ...
                <Text style={styles.description}>
                  {defaultServiceData.description}
                </Text>
              </View>
              ...
              <View style={styles.pricingSection}>
                ...
                {serviceData?.items &&
                  serviceData.items.map((item: any, index: number) => {
                    const itemId = `item_${index}`;
                    const quantity = serviceQuantities[itemId] || 0;
                    const itemPrice =
                      typeof item.price === "number"
                        ...
                        <View style={styles.quantitySelector}>
                          ...
                          <View style={styles.quantityControls}>
                            <TouchableOpacity
                              ...
                              onPress={() =>
                                handleItemQuantityChange(itemId, -1)
                              }
                              disabled={quantity === 0}
                            >
                              <Ionicons
                    ...
                    extrasList.map((extra: any, index: number) => {
                      ...
                      return (
                        <View
                          ...
                          style={[
                            styles.extraCard,
                            isActive && styles.extraCardActive,
                          ]}
                        >
                          <View style={styles.serviceHeader}>
                            <Text style={styles.serviceName} numberOfLines={2}>
                          ...
                          <View style={styles.quantitySelector}>
                            ...
                            <View style={styles.quantityControls}>
                              <TouchableOpacity
                                ...
                                onPress={() =>
                                  handleItemQuantityChange(extraKey, -1)
                                }
                                disabled={quantity === 0}
                              >
                                <Ionicons
        ...
        <View
          style={[
            styles.actionBar,
            { bottom: TOTAL_TAB_BAR_HEIGHT + insets.bottom },
          ]}
        >
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                  ...
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Ionicons
                        ...
                        size={36}
                        color={
                          star <= reviewRating ? COLORS.gold : COLORS.border
                        }
                      />
                    </TouchableOpacity>
                  ))}
              ...
              <View style={styles.modalActions}>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.modalSend,
                    (!reviewText.trim() || reviewSubmitting) &&
                ...
                <View style={styles.imageNavigation}>
                  ...
                  >
                    <Ionicons
                      name="chevron-back"
                      size={22}
                      color={(previewMedia.index || 0) === 0 ? "#666" : "#fff"}
                    />
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  fullScreenImage: {
    width: screenWidth,
    height: screenHeight * 0.85 + 30,
  },
  fullScreenVideo: {
    width: screenWidth,
    height: screenHeight * 0.8 + 30,
  },
