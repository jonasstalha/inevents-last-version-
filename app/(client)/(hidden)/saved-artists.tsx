...
export default function SavedArtistsScreen() {
  ...
  // Resolve saved artists by IDs from both local mock data and Firebase.
  React.useEffect(() => {
    let isCancelled = false;

    const loadSavedArtists = async () => {
      const localArtists = getSavedArtists();
      const artistMap = new Map(localArtists.map((artist) => [String(artist.id), artist]));

      const missingIds = savedArtistIds.filter((artistId) => !artistMap.has(String(artistId)));

      if (missingIds.length > 0) {
        const fetchedArtists = await Promise.all(
          missingIds.map(async (artistId) => {
            try {
              return await fetchArtistById(String(artistId));
            } catch (error) {
              console.warn('Failed to fetch saved artist by ID:', artistId, error);
              return null;
            }
          })
        );
      ...
      const orderedArtists = savedArtistIds
        .map((artistId) => artistMap.get(String(artistId)))
        .filter(Boolean) as any[];

      console.log('Retrieved saved artists:', orderedArtists);
      if (!isCancelled) {
        setSavedArtists(orderedArtists);
      }
    };
