...
export default function EventApp() {
  ...
  onPress={() => router.push({ pathname: '/(client)/(hidden)/artist', params: { artistId: artist.id.toString() } })}
        activeOpacity={0.95}
      >
    ...
    if (isPopupVisible && popupContent === 'artists') {
      fetchArtistsFromFirebase()
        .then((firebaseArtists) => {
          // Ensure all required fields exist for Artist type
        ...
        .catch(() => fetchArtists().then((fallbackArtists) => {
          const mapped = fallbackArtists.map((a: any, i: number) => ({
            id: a.id ?? i,
            name: a.name ?? '',
            description: a.description ?? '',
            specialty: a.specialty ?? '',
  ...
  // Fetch top tickets for Trending Now
  useEffect(() => {
  ...
  // Fetch top global services for logged-in users
  useEffect(() => {
    if (user) {
          ...
          <View style={styles.enhancedNavigationSection}>
            ...
            <View style={styles.statItem}>
              ...
              {activeSection === 'events' && (
                ...
                {/* Top Services for logged-in users, filtered by category, at the bottom with safe area */}
                {user && topGlobalServices.length > 0 && (
                  <View style={[styles.enhancedEventsSection, { marginBottom: 32, paddingBottom: 48 }]}> 
                    <View style={styles.enhancedSectionHeader}>
                      <View>
                        <Text style={styles.enhancedSectionTitle}>Your Top Services</Text>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ paddingLeft: 8, marginBottom: 8 }}>
                      {(() => {
                        const filtered = (activeCategory === 'All' || activeCategory === 'all')
                          ? topGlobalServices
                          : topGlobalServices.filter(service => {
                              // If location/category is missing, don't filter out
                              if (!service.location) return true;
                              return service.location.toLowerCase().includes(activeCategory.toLowerCase());
                            });
                        if (!filtered || filtered.length === 0) {
                          return (
                            <View style={{ padding: 24, alignItems: 'center', justifyContent: 'center' }}>
                              <Text style={{ color: '#64748b', fontSize: 16 }}>No services found for this category.</Text>
                            </View>
                          );
                        }
                            ...
                            >
                              ...
                              <View style={{ padding: 18 }}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
                                  ...
                                  <Text style={{ color: '#64748b', fontSize: 13, marginLeft: 10 }}>{service.orders} orders</Text>
                                </View>
                ...
                {/* Removed duplicated Top Services section */}
                </ScrollView>
              )}
              </View>
          </View>
