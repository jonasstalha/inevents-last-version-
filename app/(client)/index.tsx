...
export default function EventApp() {
  ...
  const [eventCount, setEventCount] = useState<number>(dummyTickets.length);
  const [artistCount, setArtistCount] = useState<number>(artists.length);
  ...
  const navigationItems = useMemo(() => [
    ...
    },
    {
      ...
      onPress: () => {
        ...
        router.push('/(client)/(hidden)/saved-artists');
      },
    },
    {
      id: 'tickets',
          ...
          <View style={[styles.serviceImageContainer, { backgroundColor: cardGradients[gradientIndex][0] }]}> 
            {transformedService.image && typeof transformedService.image === 'string' && transformedService.image.startsWith('http') ? (
              <View style={{ width: '100%', height: '100%', borderRadius: 20, overflow: 'hidden' }}>
                <Animated.Image
                  source={{ uri: transformedService.image }}
...
>
  ...
  // Load counts from real Firebase data once on mount
  useEffect(() => {
    fetchAllTickets()
      .then((tickets) => {
        setRealTickets(tickets);
        setEventCount(tickets.length);
      })
      .catch((error) => {
        console.error('Error fetching all tickets for event count:', error);
        setEventCount(dummyTickets.length);
      });
    ...
    fetchArtistsFromFirebase()
      .then((firebaseArtists) => {
        ...
        setRealArtists(mapped);
        setArtistCount(mapped.length || savedArtists.length || artists.length);
      })
      .catch(() => fetchArtists().then((fallbackArtists) => {
        const mapped = fallbackArtists.map((a: any, i: number) => ({
          id: a.id ?? i,
  ...
  useEffect(() => {
    const savedCount = getSavedArtists().length;
    setArtistCount(savedCount > 0 ? savedCount : (realArtists.length || artists.length));
  }, [savedArtists, realArtists.length, getSavedArtists, artists.length]);
      ...
      <View style={{ flex: 1, paddingBottom: 20 }}>
        ...
        <Animated.View style={[styles.enhancedHeader, {
          ...
          <View style={styles.enhancedNavigationSection}>
            ...
            <View style={styles.statItem}>
              <Text style={styles.statNumber}>{artistCount}</Text>
