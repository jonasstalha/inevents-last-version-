...
const CARD_ACCENT_COLORS = ["#4f46e5", "#667eea", "#64748b"];
  ...
  return (
    <View style={skeletonStyles.card}>
      ...
      <View style={skeletonStyles.body}>
        <Animated.View
          style={[skeletonStyles.line, skeletonStyles.lineTitle, { opacity }]}
        />
        <Animated.View
          style={[skeletonStyles.line, skeletonStyles.lineMid, { opacity }]}
        />
        <Animated.View
          style={[skeletonStyles.line, skeletonStyles.lineShort, { opacity }]}
        />
...
const ServiceCard = React.memo(
  ({ service, onPress, onShare, index }: ServiceCardProps) => {
    ...
    const ratingText =
      totalRaters > 0 ? (service.totalRating / totalRaters).toFixed(1) : "New";
        ...
        >
          ...
          >
            ...
            <View style={styles.priceBadge}>
              <Text style={styles.priceBadgeText}>{service.basePrice} MAD</Text>
            </View>
          </View>
          ...
          <View style={styles.serviceContent}>
            ...
            <Text style={styles.serviceDescription} numberOfLines={2}>
              {service.description ||
                "Contactez ce prestataire pour en savoir plus."}
            </Text>
...
const StoryImageTemplate = React.memo(
  ({
    ...
    const totalRaters = service.totalRaters || 0;
    const rating =
      totalRaters > 0 ? (totalRating / totalRaters).toFixed(1) : "N/A";
    ...
    return (
      <View ref={storyRef} style={storyStyles.canvas} collapsable={false}>
        {/* Background */}
...
export default function SearchScreen() {
  ...
  const loadMore = useCallback(async () => {
    if (loadingMoreRef.current || !hasMoreRef.current || !lastDocRef.current)
      return;
  ...
  const handleGigPress = useCallback(
    (gigId: string) => router.push(`/(client)/(hidden)/gig/${gigId}`),
    [router],
  );
            ...
            <View style={styles.emptyContainer}>
              <Animatable.View animation="fadeInUp" duration={400}>
                <View style={styles.emptyIconWrap}>
                  <FontAwesomeIcon icon={faSearch} size={28} color="#c4c4c6" />
                </View>
          ...
          <View style={styles.filterModalContent}>
            ...
            <View style={styles.ratingOptions}>
              {[1, 2, 3, 4, 5].map((star) => (
                <TouchableOpacity
                  key={star}
                  onPress={() => setMinRating((r) => (r === star ? 0 : star))}
              ...
              <View style={styles.shareOptions}>
                {[
                  ...
                  >
                    ...
                    >
                      <FontAwesomeIcon icon={opt.icon} size={24} color="#fff" />
                    </View>
